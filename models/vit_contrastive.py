...
class LitClassifier(pl.LightningModule):
    ...
    def shared_step(self, batch):
        ...
        if self.loss_ver == 'v0':
         ...
         # loss version v1 and v2
            ...
            elif self.loss_ver == 'v2':
                loss = self.nt_xent_loss_masking(z1, z2, z3, self.temperature)
    ...
    # Loss version 2 implementation
    def nt_xent_loss_masking(self, out_1, out_2, out_3, temperature, eps=1e-6):
        """
            Computes the NT-Xent loss for temporal windows.

            Args:
            - representations: Tensor of shape [batch_size, dim], where each row is the representation of a frame.
        ...
        representations = torch.cat([out_1, out_2, out_3], dim=0)

        
        batch_size, dim = representations.shape

        # Compute the similarity matrix for all pairs
        similarity_matrix = torch.mm(representations, representations.t().contiguous())  # Shape: [batch_size, batch_size]
        similarity_matrix /= temperature  # Scale by temperature
        similarity_matrix = torch.exp(similarity_matrix)  # Apply exponential
        
        # Create the mask to identify positive and negative pairs
        mask = torch.zeros_like(similarity_matrix, dtype=torch.bool)

        # Identify positive pairs within each temporal window
        for i in range(batch_size):
            # Assume each temporal window is of size 3
            window_start = (i // 3) * 3  # Find start index of the temporal window FIXME: hardcoded window size used
            for j in range(window_start, window_start + 3):  # Iterate over window frames
                mask[i, j] = True  # Mark as positive pair, including self-similarity

        #print(mask)

        # Compute positives and negatives
        # Positive similarities (masked values from similarity_matrix)
        positives = similarity_matrix[mask]  # Shape: [number of positives]


        # Negative similarities (all except positives)
        negatives = similarity_matrix.sum(dim=1, keepdim=True) - positives.sum()  # Subtract positive similarities

        # compute NT-Xent loss -> Numerator: Positive similarities
        numerator = positives.sum(dim=0)

        #print("pos - ", numerator)

        # Denominator: Sum of all similarities (positives + negatives)
        denominator = negatives + positives.sum(dim=0)
        denominator = denominator.sum(dim=0)

        #print("neg - ", denominator + eps)
